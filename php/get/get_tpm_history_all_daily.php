...
<?php
  ...
  $query_masch = "SELECT * FROM sd_masch_list";
  $result_masch = mysqli_query($con,$query_masch);


  $all_masch_data = [];

  while($r_masch = mysqli_fetch_assoc($result_masch)) {

    $query = "SELECT *,date(insert_time) as only_date, month(insert_time) as month, TIME_FORMAT(`insert_time`, '%H:%i') as time_task, FIND_IN_SET (TIME_FORMAT(`insert_time`, '%H:%i'),task_occur_time) as shift  from v_full_data_tpm WHERE task_occur=0 and masch_id='".$r_masch['m_id']."'";
    ...
    $data = array('daily' => $task_list_daily, 'weekly'=>$task_list_weekly, 'monthly'=>$task_list_monthly, 'other'=>$task_list_other, 'dates'=>$arr, 'weeks'=>$arr_week, 'months' => $arr_months, 'shiftly'=>$task_list_shift,'shifts'=>$arr_shift, 'shifts_nested'=>$arr_shift_nested, 'other'=>$task_list_other );

    $all_masch_data[] = array('masch_id' => $r_masch['m_id'],'masch_name' => $r_masch['name'], 'data'=> $data);


  }
  //$result->free();
  echo json_encode($all_masch_data);
  mysqli_close($con);
