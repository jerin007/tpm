...
<?php
  ...
  while($r_masch = mysqli_fetch_assoc($result_masch)) {
    ...
    $query = "SELECT *,date(insert_time) as only_date, month(insert_time) as month, TIME_FORMAT(`insert_time`, '%H:%i') as time_task, FIND_IN_SET (TIME_FORMAT(`insert_time`, '%H:%i'),task_occur_time) as shift  from v_full_data_tpm WHERE task_occur=4 and masch_id='".$r_masch['m_id']."'";
